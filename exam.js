...
function mission1(values)
{
   const name = values.commandStructure.chiefOfStaff.name
   const rank = values.commandStructure.chiefOfStaff.rank
   const phone = values.commandStructure.chiefOfStaff.contact.phone
    return [name,rank,phone]
...
function mission2(values)
{
    const lenSoldier = values.personnel.length
    return lenSoldier
}
